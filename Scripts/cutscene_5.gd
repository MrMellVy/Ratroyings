...
func _on_dialogue_event(event_name: String) -> void:
	if event_name == "attack_boss_move_back":
		...
		_change_camera($WorldCamera2, 1.0, 5.0)
		await move_player_to_target($Movetarget1)
		
		$Player/AnimatedSprite2D.play("dash_attack")
		$BOSSprite.play("hurt")
		...
		_change_camera($WorldCamera3, 0.0, 30.0, 0.3)
		
		await get_tree().create_timer(1.0).timeout
		$AnimationPlayer.play("boss_defeated")
		await $AnimationPlayer.animation_finished

		_change_camera($WorldCamera, 0.0)
		$Enemy2.flip_h = true
		$AnimationPlayer.play("enemy_defeated")
		await $AnimationPlayer.animation_finished
		await get_tree().create_timer(0.5).timeout
		_change_camera($WorldCamera4, 0.0, 0.2, 1000.0)
		move_player_to_target($Movetarget4)
	...
	if event_name == "run_away":
		...
		move_player_to_target($Movetarget2)
		move_support_to_target($Movetarget3)
